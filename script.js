...
function sideMenu(side) {
  ...
  var animation = bodymovin.loadAnimation({
    ...
    loop: true,
    autoplay: true,
    path: '97639-coding.json' // Replace with the path to your downloaded animation JSON file
  });
  ...
  form.addEventListener('submit', function(event) {
    ...
    // You can use JavaScript's Fetch API or submit the form data to a backend server using AJAX
    
    // Example of displaying a success message after form submission
    const successMessage = document.createElement('p');
    successMessage.textContent = 'Application submitted successfully!';
    form.appendChild(successMessage);
  });
  
  // JavaScript function to reduce input width
  ...
  reduceInputWidth(); // Call the function to reduce input width

// JavaScript function to shift the form to the right
  function shiftFormRight() {
    const form = document.getElementById('jobApplicationForm');
    form.classList.add('shifted-form');
  }
  
  shiftFormRight(); // Call the function to shift the form to the right